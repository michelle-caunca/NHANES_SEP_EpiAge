...
get_residuals <- function(y){
  ...
  residuals <- svyglm(formula, design=dstrat, na.action=na.exclude)$residuals
  residuals
}
...
get_plot <- function(z){
  ...
  p <- ggviolin(all_residuals_ses, 
    ...
    stat_compare_means(
      comparisons = list(c("Lower", "Higher"), 
                         c("Lower", "Middle"),
                         c("Middle", "Higher")),
      label = "p.signif") + 
...
# FIGURE 2
epigen_strat <- c("GrimAgeMort", "GrimAge2Mort", "HorvathTelo")
...
ses_strat <- rbind(ses_men, 
  ...
  mutate(model = factor(model,
                        levels = c("White", "Black", "Hispanic", "Men", "Women")))
view(ses_strat)
...
make_fp <- function(x) {
  ...
  ses_strat_fp <- ses_strat %>%
    ...
    scale_color_manual(values = c("#1B9E77", "#D95F02", "#7570B3", "#E7298A", "#66A61E"),
                       name = "Model",
                       breaks = c("White", "Black", "Hispanic", "Men", "Women"),
...
fp_tbl_horvathtelo <- fp_tbl_all[[3]]
ggsave("fp_tbl_horvathtelo.jpg",
       plot = fp_tbl_horvathtelo,
       unit = "in",
       width = 8,
       height = 6, 
       dpi = 300)
